...
enum AppMode {
    ...
    APP_MODE_JAMMER,             // Mode Jammer Sederhana (Wi-Fi/BT/BLE/All)
...
enum JammerTarget {
    JAM_TARGET_WIFI = 0,         // Target Wi-Fi 2.4 GHz (Ch 1 - 73 / 2401 - 2473 MHz)
    JAM_TARGET_BT = 1,           // Target Bluetooth Classic + BLE (Ch 2 - 80 / 2402 - 2480 MHz)
    JAM_TARGET_BLE_ADV = 2,      // Target BLE Advertising Only (Ch 2, 26, 80)
    JAM_TARGET_ALL = 3           // Target Seluruh Band 2.4 GHz (Ch 0 - 125)
};
...
struct AppState {
    ...
    unsigned long hopDwellUs = DEFAULT_DWELL_US;
    uint8_t randomPayload[PAYLOAD_SIZE];
    ...
    int inspectedChannel = 36;               // Default Wi-Fi Ch 6 (2437 MHz / RF Ch 37)
